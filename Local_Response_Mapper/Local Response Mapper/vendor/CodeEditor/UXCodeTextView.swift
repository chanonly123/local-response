...
final class UXCodeTextView: UXTextView {
  ...
  /// What `applyNewTheme` last actually applied.
  ///
  /// Local change: upstream reloads the theme — a css file read off disk and
  /// parsed — and re-applies a font across the whole text storage on every
  /// update, which relays out the document. Recorded here so an update asking
  /// for what is already on screen can do nothing at all.
  private var appliedThemeName : CodeEditor.ThemeName?
  ...
  func applyNewTheme(_ newTheme: CodeEditor.ThemeName) -> Bool {
    // Local change: compared against what was applied. Upstream compares
    // `themeName`, which nothing ever assigns, so this never held.
    guard appliedThemeName != newTheme else { return false }
  ...
  {
    // Local change: nothing below is worth doing twice for the same theme and
    // size, and all of it is expensive — the css is re-read from disk and
    // parsed, and the font built from it is applied over the whole text
    // storage, relaying out the document. This runs on every update.
    let wantedTheme = newTheme ?? themeName
    if appliedThemeName == wantedTheme, appliedFontSize == newSize {
      return false
    }
